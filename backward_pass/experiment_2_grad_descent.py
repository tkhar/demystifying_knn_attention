...
while iteration != num_iterations:
    ...
    loss_fast = torch.mean(0.5 * (O_fast ** 3) + (O_fast ** 2) + 1)
    loss = torch.mean(0.5 * (O ** 3) +  (O ** 2) + 1)
    ...
    time_start = time.time()
    dQ_fast = fast_grad_Q(Q_copy, K_copy, V_copy, O.grad, epsilon=0.25, delta=0.5)
...
plt.ylabel('Loss')
plt.title('Loss non-convex: 0.5 * (O ** 3) + (O ** 2) + 1')
plt.show()
